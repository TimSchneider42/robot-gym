...
class JointVelocitySensor(ContinuousSensor["BaseTask"], JointSensor, ABC):
    ...
    def _get_limits(self) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
        ...
        # velocity properly.
        upper_limits = self.observed_robot_component.joint_velocity_limits * 2
        return {
    ...
    def _reset_unnormalized(self) -> Dict[str, np.ndarray]:
        return {
            "{}_joint_vel".format(self.name_prefix): np.zeros((len(self.observed_robot_component.joint_intervals),))
        }
