# Code copied from pybullet and adapted
...
class UrdfEditor(object):
    ...
    def writeJoint(self, file, urdfJoint, precision=5):
        ...
        str = '<limit effort="{:.{prec}f}" lower="{:.{prec}f}" upper="{:.{prec}f}" velocity="{:.{prec}f}"/>'.format(
            urdfJoint.joint_max_force, urdfJoint.joint_lower_limit, urdfJoint.joint_upper_limit,
            urdfJoint.joint_max_velocity, prec=precision)
        file.write(str)
